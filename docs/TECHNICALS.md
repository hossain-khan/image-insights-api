...
```text
darkness_score = round((avg_luminance / 255) * 100)
```
...
```bash
curl -X POST http://localhost:8080/v1/image/darkness \
  -F "image=@photo.png"
```
...
app = FastAPI(title="Image Darkness API")

@app.post("/v1/image/darkness")
async def analyze_darkness(image: UploadFile = File(...)):
...
```bash
docker build -t image-darkness-api .
docker run -p 8080:8080 image-darkness-api
```
...
```
/v1/image/darkness?method=rec709|avg|hsv
```
